...
/* 32 bit ELF emulation code for h8300elf
...
static void gldh8300elf_before_parse (void);
static void gldh8300elf_after_open (void);
static void gldh8300elf_before_allocation (void);
static bfd_boolean gldh8300elf_place_orphan (asection *s);
static void gldh8300elf_finish (void);


static void
gldh8300elf_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldh8300elf_before_parse (void)
{
  ldfile_set_output_arch ("h8300", bfd_arch_h8300);
...
static bfd_boolean
gldh8300elf_load_symbols (lang_input_statement_type *entry)
{
  int class = 0;
...
static void
gldh8300elf_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldh8300elf_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldh8300elf_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needed != NULL)
	{
	  ...
	  lang_for_each_input_file (gldh8300elf_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldh8300elf_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldh8300elf_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldh8300elf_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldh8300elf_try_needed (&needed, force))
	return TRUE;
...
static void
gldh8300elf_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldh8300elf_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldh8300elf_write_build_id_section (bfd *abfd)
{
...
gldh8300elf_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
    {
      ...
      size = gldh8300elf_id_note_section_size (abfd, &link_info);
      if (size == 0)
	{
      ...
      else
	{
	  ...
	  if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		...
		= &gldh8300elf_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldh8300elf_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldh8300elf_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldh8300elf_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldh8300elf_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldh8300elf_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldh8300elf_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldh8300elf_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldh8300elf_find_exp_assignment (exp->binary.lhs);
      gldh8300elf_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldh8300elf_find_exp_assignment (exp->trinary.cond);
      gldh8300elf_find_exp_assignment (exp->trinary.lhs);
      gldh8300elf_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldh8300elf_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldh8300elf_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldh8300elf_find_exp_assignment (s->assignment_statement.exp);
}
...
static void
gldh8300elf_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  ...
  lang_for_each_statement (gldh8300elf_find_statement_assignment);
...
static bfd_boolean
gldh8300elf_open_dynamic_archive
...
static bfd_boolean
gldh8300elf_place_orphan (asection *s)
{
...
static void
gldh8300elf_finish (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);

  gldh8300elf_map_segments (need_layout);
  finish_default ();
}

static char *
gldh8300elf_get_script (int *isfile)
{
  *isfile = 0;
...
; }
#define gldh8300elf_add_options NULL
#define gldh8300elf_handle_option NULL
#define gldh8300elf_list_options NULL

struct ld_emulation_xfer_struct ld_h8300elf_emulation =
{
  gldh8300elf_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldh8300elf_after_open,
  after_allocation_default,
  set_output_arch_default,
  ldemul_default_target,
  gldh8300elf_before_allocation,
  gldh8300elf_get_script,
  "h8300elf",
  "elf32-h8300",
  gldh8300elf_finish,
  NULL,
  gldh8300elf_open_dynamic_archive,
  gldh8300elf_place_orphan,
  NULL,
  NULL,
  gldh8300elf_add_options,
  gldh8300elf_handle_option,
  NULL,
  gldh8300elf_list_options,
  gldh8300elf_load_symbols,
  NULL,
  NULL
};
